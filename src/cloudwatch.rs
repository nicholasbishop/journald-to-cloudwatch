...
pub fn upload_thread(conf: Configuration, rx: mpsc::Receiver<InputLogEvent>) {
    ...
    loop {
        ...
        // If it's been over a second since we last flushed, flush now
        if let Some(last_flush_time) = state.last_flush_time {
            if Utc::now() - last_flush_time > chrono::Duration::seconds(1) {
                state.flush();
            }
        } else i{
            // If we've never flushed, go ahead and flush now
            state.flush();
        }
    }
}
